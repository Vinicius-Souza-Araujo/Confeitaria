  ...
  async function get_enderenco() {
    if (user.data.id !== "") {
      ...
      if (response.ok) {
        ...
        const enderecosEntrega = data.filter(
          (endereco) => endereco.tipo === "ENTREGA"
        );
        console.log(enderecoEntrega);
        setEnderecoEntregaLogin(enderecosEntrega);
      } else {
    ...
    <div className="body-carrinho">
      ...
      <div className="remover-todos-itens">
        <button className="botao-remover-itens" onClick={handleRemoveAllItems}>
          <img src="src\assets\lixo.svg" width={30} />
          ...
          <div className="produtos-carrinho">
            <div className="itens-carrinho">
              {cartState.cartItems.map((item, index) => {
                const imagemTrue = item.imagens.find(
                  (imagem) => imagem.flag === false
                );
                ...
                return (
                  <div key={index} className="card-item-pedidos">
                    ...
                    <div>
                      ...
                      <button
                        className="continuar-compra"
                        onClick={() => incrementar(index, dispatch)}
                      >
                        {">"}
                      </button>
                      <button
        ...
        <div className="resumo-pedido">
          ...
          <div className="total">
            ...
            <div className="botoes-pagamento">
              {user.data.grupo === "CLIENTE" ? (
                <div>
                  <div>
                    <button onClick={geradoPedido} className="botaoAzul">
                      Pagamento
                    </button>
                    <Link to="/home">
                  ...
                  <label htmlFor="selecionar-enderenco-entrega">
                    {" "}
                    Selecione enderenço de entrega{" "}
                  </label>
                  <select
                    onChange={(e) => {
                      const selectedIndex = e.target.value - 1;
                      const selectedEndereco =
                        enderecoEntregaLogin[selectedIndex];

                      if (selectedEndereco) {
                        setEnderecoEntrega(selectedEndereco);
                        setCep(selectedEndereco.cep.replace("-", ""));
                      }
                    }}
                  >
                ...
                <div className="sem-login">
                  ...
                  <div className="informe-cep-car">
                    ...
                    <button className="botaoRosa" onClick={() => getCEP(cep)}>
                      Calcular frete
                    </button>
                  </div>
                </div>
          ...
          <div className="tipo-entrega">
            ...
            >
              ...
              <option value="entrega-transportadora">
                Entrega Transportadora
              </option>
            </select>

            <Link to='/alterarEnderenco'><button className="botaoRosa">Cadastrar enderenço</button></Link>
          </div>
          ...
          <div className="resultados-cep">
            {enderecoEntrega && enderecoEntrega.logradouro ? (
              <div>
                <p>
                  {enderecoEntrega.logradouro}, {enderecoEntrega.bairro} - CEP:{" "}
                  {cep}
                </p>
                <p>
                  {enderecoEntrega.localidade} -{" "}
                  {enderecoEntrega.uf.toUpperCase()}
                </p>
