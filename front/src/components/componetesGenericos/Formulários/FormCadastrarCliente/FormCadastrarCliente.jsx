...
function FormCadastrarCliente() {
    ...
    async function getCEPE(cep) {
      ...
      setCidadeE(data.localidade)
      setBairroE(data.bairro)
      setComplementoE(data.complemento)
      setLogradouroE(data.logradouro)
      setUfE(data.uf.toLowerCase())
    }
    ...
    <fieldset>
      ...
      <select name="uf" id="uf" value={uf} 
        ...
        <InputMask
              ...
              required />
        <button onClick={() => getCEPE(cepE)}>Consultar CEP</button>
        <label htmlFor="logradouro">Logradouro</label>
        ...
        <label htmlFor="complemento">Complemento</label>
        <input type="text" id="complementoE" value={complementoE} onChange={(event)=>setComplementoE(event.target.value)} placeholder="digite o bloco e número do apartamento, ou algum ponto de referência" />
